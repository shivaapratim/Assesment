...
function Authorization(roles = []){ //to check if the user is authorized or not!
    return function (req,res,next){ //middleware function
        ...
        if(!token) return res.redirect('/shivang/signin');//if token is not present redirect to signin page

        const User = getUser(token); //getting the user from the token
        if(!User) return res.redirect('/shivang/signin'); //if user is not present redirect to signin page
